...
이처럼 중앙 집중형 CoreDNS 구조는 소규모 개발 환경에서는 문제없이 동작하지만, **프로덕션 규모로 확장될 때 근본적인 아키텍처 한계**를 드러내게 됩니다. 이 문제들을 근본적으로 해결하기 위해, 본 문서에서는 AKS의 노드-레벨 DNS 프록시 기능인 **LocalDNS** 도입을 제안합니다.
...
Pod의 DNS 쿼리는 **동일 노드의 LocalDNS로 먼저 전달**되며, 캐시 히트 시 즉시 응답합니다. 캐시 미스일 경우에만 CoreDNS Pod으로 TCP를 통해 전달됩니다.
