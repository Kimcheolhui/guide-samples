...
- **비교 축**: 노드 수(5/10) × Pod당 QPS(20/40/80/160) — 총 16개 조건
...
5. **쿼리 손실에 부정적 영향 없음**: Baseline에서 일부 run에서 소량의 쿼리 손실(수백만 건 중 1~2건)이 관측되었으나, LocalDNS 적용 후에도 손실 수준은 동일하게 유지되었습니다. LocalDNS 도입이 쿼리 안정성에 부정적 영향을 미치지 않습니다.
